...
-- Fix 1: Remove user-facing INSERT/UPDATE policies on ai_usage (SECURITY DEFINER function handles all writes)
...
-- Fix 2: Add auth.uid() check to increment_ai_usage to prevent quota DoS
...
BEGIN
  -- Ensure callers can only affect their own usage
