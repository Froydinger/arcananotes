...
import arcLogo from '@/assets/arc-notes-logo.png.asset.json';
    ...
    try {
      ...
      if (resp.status === 429) {
        ...
        if (data?.limit_reached) {
          toast.error('Arc Matrix is busy. Try again in a moment.', { duration: 5000 });
        } else {
    ...
    <>
      ...
      {!isOpen && (
        ...
        >
          ...
          >
            <img src={arcLogo.url} alt="Arc" width={1024} height={1024} className="h-8 w-8 rounded-full object-cover" />
                ...
                {showHistory ? (
                  ...
                  <div className="relative">
                    <div className="w-7 h-7 rounded-full flex items-center justify-center bg-accent/10 border border-accent/20">
                      <img src={arcLogo.url} alt="Arc" width={1024} height={1024} className="h-5 w-5 rounded-full object-cover" />
                    </div>
                  ...
                  {messages.length === 0 ? (
                    <div className="flex flex-col items-center justify-center h-full text-center px-4 gap-4">
                      <div className="w-16 h-16 rounded-2xl flex items-center justify-center bg-accent/10 border border-accent/15">
                        <img src={arcLogo.url} alt="Arc" width={1024} height={1024} className="h-10 w-10 rounded-full object-cover" />
                      </div>
                      <div>
                      ...
                      <div key={i} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                        {msg.role === 'assistant' && (
                          <div className="w-6 h-6 rounded-full shrink-0 mr-2 mt-1 flex items-center justify-center bg-accent/10 border border-accent/20">
                            <img src={arcLogo.url} alt="Arc" width={1024} height={1024} className="h-4 w-4 rounded-full object-cover" />
                          </div>
                        )}
                        <div
